...
### 0.12.7 — Plans worth reading, and a card that stays in its half of the window
...
*Status: built & tested (`cargo test --lib` 184 passing, `npx tsc --noEmit` clean, `vite build` green); not yet runtime-tested against a live model. The step gains `detail`/`acceptance` and the plan gains `context`/`doc_path` in [plans.rs](../src-tauri/src/plans.rs) over migration [036](../src-tauri/migrations/036_plan_depth.sql); the drafting and reading tools are [tools/plan_mode.rs](../src-tauri/src/tools/plan_mode.rs); the card is [PlanReview.tsx](../src/components/Chat/PlanReview.tsx).*
