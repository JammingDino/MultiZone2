...
The leader is the only agent that can talk to you — sub-agent `ask_user` calls are suppressed — and the whole leader → sub-agent call tree is inspectable inline in the stack tracer, with every subchat transcript openable. Exporting the chat keeps them: sub-agent conversations are nested under the turns that spawned them in both the Markdown and PDF exports, with each turn attributed to the zone that wrote it, so a prompt from the leader is never presented as something you asked for. Turn it off under **Settings → Appearance → Chat export** when you only want the conversation you were part of.
