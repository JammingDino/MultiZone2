...
- [ ] CI job on every push: `npm run build` + `cargo test --lib`. There is currently no workflow that runs on an ordinary push, so nothing is verified between releases
...
- [ ] Fixture test for `scripts/rewrite-updater-manifest.mjs` — the private-repo URL rewrite is the piece a local updater test cannot cover
