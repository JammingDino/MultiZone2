...
- [~] Terminals have no Tauri command to route — they are a tool group, driven from inside a turn — so the drift test finds nothing to fill there. Reaching a live terminal from outside the app is a separate design question, not a missing route
...
- [ ] Runtime-test it against the running app: ask a zone to change the theme, create a zone, and diagnose a deliberately broken API setting
...
- [ ] Runtime-test the title bar on a touchscreen and with a stylus — drag, double-tap to maximize, and the three window buttons. The diagnosis is from the injected script and tao's Windows `drag_window`; the fix is unverified on real touch hardware
...
- [ ] Runtime-test: drag the window by its title bar with a finger and with a pen — **not verifiable off-device**, no touchscreen or Tauri shell in the harness
...
*How each open item below can actually be closed — including the three that a manual pass cannot honestly close at all — is worked through in [TEST_STRATEGY.md](TEST_STRATEGY.md). Summary: build the mock streaming provider first (it turns "no dropped tokens" from an unfalsifiable claim into a diff, and needs no provider, key or network), add a CI job that runs `npm run build` and `cargo test --lib` on push (37 files of Rust tests exist and nothing runs them), and test the updater against a local two-build loop rather than the release pipeline, where a build is a publish.*
...
- [x] CI job on every push and PR ([ci.yml](../.github/workflows/ci.yml)): `npm run build` (which runs `tsc -b` first), `npm test`, and `cargo test --lib` on Windows. `cargo fmt --check` runs advisory-only — the tree is not rustfmt-clean and reformatting it wholesale would bury real changes for a long time afterwards. **Both jobs failed on their first real run (v0.13.3) and are fixed:** the runner was on Node 20, where `node --test` cannot expand a glob (now 22, and release.yml with it, so a release is not built on a different major than CI verifies); and `tauri::generate_context!()` resolves `frontendDist` at compile time, so the Rust job now writes a placeholder `dist/index.html` — the lib tests read no frontend asset, and building the real one there would add a duplicate `npm ci` to a 13-minute job. Both reproduced locally before fixing. *Still unverified: whether the workflow is green, which needs another push*
