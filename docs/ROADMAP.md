...
**Done when:** a new user can read the zone editor's tool list and understand every entry without guessing; a model can rename and move files as fluently as it creates them; a model can find an exact string in a project without an embedding round-trip; a zone can write a skill that measurably improves its own later runs; a long conversation stays coherent past the context window instead of quietly forgetting how it began; no tool reports a failure as an ordinary empty result; and a multi-step turn ends with an answer rather than with the last tool result.
