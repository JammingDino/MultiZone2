...
- [ ] **Blocker, and it is a settings page, not engineering:** add `TAURI_SIGNING_PRIVATE_KEY` and its password to repo secrets. Until they exist every published build ships without a signed manifest, and every existing install silently stops seeing updates
- [x] CI job on every push and PR ([ci.yml](../.github/workflows/ci.yml)): `npm run build` (which runs `tsc -b` first), `npm test`, and `cargo test --lib` on Windows. `cargo fmt --check` runs advisory-only — the tree is not rustfmt-clean and reformatting it wholesale would bury real changes for a long time afterwards. **Unverified until something is pushed:** the workflow has never run
...
- [~] Auto-updater: Tauri updater plugin wired to GitHub releases — *built in 0.9.12*: plugin registered, signed `latest.json` published by CI alongside the installers, Settings → Data carries check / download-with-progress / install / restart. Two blockers before it functions: the `TAURI_SIGNING_PRIVATE_KEY` secrets must be added to the repo, and end-to-end verification needs two successive builds (install N, publish N+1, watch it update)
