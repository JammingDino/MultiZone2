...
- [ ] **Tool approvals inside subchats never reach the user** — subchats are read-only in the UI, so the approval banner isn't shown; a sub-agent tool call needing approval waits ~5 min and is auto-denied, silently stalling the sub-agent. Safe today only if sub-agent zones stick to auto-approved/safe tools. Fix: surface subchat approval prompts somewhere actionable (parent chat, or an observable approval UI in the subchat view). Introduced in 0.5.1 (spawn-subagent tools). *Sharper since 0.9.10*: background sub-agents can stall five at once, and the SWE-Bench panel's shell-running members need auto-approval set to "Everything" — which is why the SWE Lead's library blurb says so
...
### 0.9.10 — Parallel sub-agents & the SWE-Bench panel
...
- [x] **SWE-Bench Panel** — a leader plus seven specialists ([defaultZones.ts](../src/lib/defaultZones.ts)), designed to run on *one* model at eight temperatures (0.0–0.9) so the panel's value comes from independent attempts and adversarial review rather than a bigger model: SWE Lead (0.2, parallel recon → one root-cause brief → two independent patches → per-candidate verification → arbitration → applies and verifies the winner itself), Repro Engineer (0.1), Code Cartographer (0.3), Patch Author Careful (0.1), Patch Author Inventive (0.9), Adversarial Reviewer (0.5), Test Runner (0.0, mechanical: baseline → apply → run → revert, raw output only), Patch Arbiter (0.2). Shared house rules (never touch tests, minimal diff, leave the tree clean, quote don't paraphrase) live in one constant rather than eight prompts; the Lead's final answer is contracted to exactly one fenced `diff` block so a harness can extract it
