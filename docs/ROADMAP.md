...
| Mobile app | **Built as 0.17.x** — Android. A Tauri mobile target that is a **remote for the desktop**, not a second app: same chats, same zones, but the machine at home runs the turn with its models, files and MCP servers. The three things that did not exist all landed — a LAN bind that is its own switch, pairing by code with per-device tokens and per-device revocation, and a pending-approval queue readable and answerable from the phone — plus mDNS, `GET /api/events` (the `listen` half of the seam, which turned out to be missing entirely), and a mobile build with every backend module compiled out, so the "nothing is stored there" promise is a `cfg` rather than a policy. LAN-only by design: no relay, no tunnel, no offline mode. iOS is unbuilt; the crate is split for it. See [CONNECTIVITY.md](CONNECTIVITY.md#part-3-as-built-017x) and [RELEASE_PLAN.md](RELEASE_PLAN.md#017x--the-phone-as-a-second-window) |
| Running chat mode (rolling window) | A per-chat mode in which each request carries only the last turn — or the last *N* turns, configurable — rather than the whole history. For a chat used as a standing tool (translate this, classify that, one-off questions to a local model all day) the transcript is a log, not context, and re-sending it is what makes the meter climb and a small model slow. Distinct from compaction, which condenses old turns into a summary and keeps them in play; here they are simply not sent. Needs: a chat-level setting with the window size, `build_message_history` honouring it, the context meter and panel saying which mode the chat is in, and a visible marker in the transcript at the window's edge so it is obvious what the model can and cannot see. Memories, project and tag context still apply — the baseline is per request, not per history. |
