...
*Also unplanned, and the biggest of these — found by discovering [Hound](https://github.com/dondai1234/master-fetch), a keyless host-driven web-research MCP server, and realising MultiZone's web search was the app's weakest link. The original `web_search` scraped a single engine (DuckDuckGo) over `reqwest`, whose rustls TLS fingerprint the engines detect as "not a browser" and answer with an anti-bot challenge — so the app's research value was low and often silently empty. Three new tools re-implement Hound's ideas simplified to MultiZone's no-headless-browser stance, credited in every file, `Cargo.toml`, and the tool registry. The old `web_search`/`extract_url` stay available (they still hold the key-based API providers — Brave/Tavily/Serper/SearXNG) but the default and curated zones now point at the smart tools.*

- [x] **Browser-emulating HTTP client** ([web_util.rs](../src-tauri/src/tools/web_util.rs)) — the actual fix. A shared `wreq` client (BoringSSL-backed) carries a real Chrome JA3/JA4 + HTTP/2 fingerprint, so keyless engine scraping and page fetches get results instead of a `202` challenge. This is the HTTP layer of Hound's approach (which uses Python's `primp`) without a stealth browser; only the smart tools use it, the rest of the app stays on reqwest/rustls. **Cost, weighed and accepted:** wreq needs a C toolchain (CMake, Perl, **NASM**, a C/C++ compiler) at build time — the native-build-dep burden whisper.cpp/tesseract were removed to avoid — and raised the workspace MSRV to 1.85. A `redirect::Policy::limited(10)` is set explicitly because wreq, unlike reqwest, follows no redirects by default
- [x] **`smart_search`** ([smart_search.rs](../src-tauri/src/tools/smart_search.rs)) — seven keyless engines queried **in parallel** (DuckDuckGo, Bing, Brave, Yandex, Ecosia, Yahoo, Wikipedia) and merged with Reciprocal Rank Fusion: a URL found by several engines is rewarded and deduped, snippets are enriched across engines, and each result reports which engines `found_by` it. One engine being blocked no longer sinks the query, and a total wipeout returns an honest per-engine failure report rather than a silent empty. Engine set chosen empirically through the emulated client from a datacenter IP — Google, Startpage, Qwant all answered a keyless client with a challenge/403 only a stealth browser clears, and Mojeek captchas non-residential IPs, so all four were dropped rather than shipped as dead engines. Yahoo's `r.search.yahoo.com/…/RU=` redirect wrappers and Bing's `ck/a` redirects are decoded
- [x] **`smart_fetch`** ([smart_fetch.rs](../src-tauri/src/tools/smart_fetch.rs)) — reads one or more pages **or PDFs** in full as clean markdown (semantic content root, chrome stripped), with optional relevance-query trimming and opt-in image URLs. Supersedes `extract_url`: adds PDF extraction (`pdf-extract`) and honest failure reasons — blocked / auth-required / not-found / rate-limited / "likely JS-rendered" — instead of a blank page, since there's no headless browser to run JS or clear an interactive challenge
- [x] **`smart_crawl`** ([smart_crawl.rs](../src-tauri/src/tools/smart_crawl.rs)) — from a seed URL, follows same-site links and reads several pages, **best-first**: with a `query`, links whose anchor text and path look most relevant are visited first (verified live — "install cargo" on rust-lang.org visited the seed, then `/tools/install`, then the install-methods page, in relevance order). Same-site only (subdomains optional), sequential to stay gentle, capped by `max_pages` (default 5, max 15). No JS, robots.txt not consulted (documented in the result)
- [x] **Wired in** — new `ToolId`s (moderate safety, like the tools they replace), `dispatch`, `ALL_TOOL_IDS`, and the frontend tool registry ([types.ts](../src/lib/types.ts), Web category, Hound credit). Default and curated zones ([defaultZones.ts](../src/lib/defaultZones.ts)) now enable `smart_search`/`smart_fetch`, with `smart_crawl` added to the Deep Researcher
- [x] **Verified** — `cargo test` green (unit tests for URL/host helpers, RRF fusion, DDG/Bing/Yahoo redirect decoding, anchor parsing); ignored live smoke tests for all three tools confirm real results end-to-end through the emulated client (all seven engines answered a sample query). Build requires NASM on PATH

---

### 0.9.8 — Traced PDF export & silent diagram repair

*Two complaints about the same thing: the app knew more than it showed. A PDF export of an agentic run contained only the model's prose — no sign that twelve tools ran, a plan was tracked, or a diagram was drawn — so the reader got conclusions with the work deleted. And a Mermaid diagram that failed to parse just sat there as a red error box, offering a button that dumped the failure back into the conversation for the user to babysit.*

- [x] **The PDF exports the whole run, not the answer** ([exportTrace.ts](../src/lib/exportTrace.ts), [export.ts](../src/lib/export.ts)) — a new pure trace module reduces the message list to ordered turns whose items are text, reasoning markers and tool calls, with tool results matched back onto the call that issued them and consecutive assistant steps collapsed into one turn. Previously `renderMessages` dropped every tool and system message and any turn without visible text, which is most of a tool-heavy run. Markdown export is unchanged
- [x] **Every tool call is a timestamped card with a plain-language summary** — `describeTool` maps ~40 built-in function names to a human label, an icon, a *subject* drawn from the arguments (the path read, the query searched, the command run, the URL fetched) and an *outcome* drawn from the result ("2 of 5 done", "120 lines read", "exit 0 · 3 lines of output", "2 results", "HTTP 404"). Unmapped and MCP tools fall back to a prettified name and the first meaningful argument, so a tool the map has never heard of still reads as a sentence. Cards carry the wall time from call to result and are colour-railed by status; failures show the error and its hint
- [x] **Reasoning is acknowledged, never exposed** — a thinking block renders as a dashed marker row with its timestamp and character count and no content, which is what the user asked for: tell me it happened, don't print the model's private notes
- [x] **Artifacts are drawn as artifacts** — `update_plan` renders as a real checklist with a progress bar and struck-through completed steps; `draw_diagram` and `plot_function` are genuinely rendered to inline SVG at export time (`renderMermaidSvg` / `renderPlotSvg`, both lifted out of their React components); `present_file` gets a file card with format badge and full path; search results become a numbered list of titles and URLs. Everything else gets a tightly clipped output preview (8 lines / 600 chars). `buildChatPrintHtml` and `exportChatPdf` are now async because of the real rendering
- [x] **A summary strip under the header** — messages, responses, tool calls (and how many failed), reasoning blocks, elapsed time, plus a chip per tool used with its call count. The skim path before reading a line
- [x] **Mermaid failures repair themselves in the background** ([diagram.rs](../src-tauri/src/commands/diagram.rs), [MermaidBlock.tsx](../src/components/Renderers/MermaidBlock.tsx)) — a `draw_diagram` result that won't parse now triggers `fix_diagram`, a one-shot completion whose entire context is the source and the parser error. It is handed a single tool and `tool_choice` forces the call (with a fenced-block fallback for providers that ignore forced choice, verified by unit tests). Nothing enters the conversation: the block shows an indeterminate progress bar, then the working diagram. `tool_choice` was added to `ChatRequest` for this
- [x] **The fix is written back over the original tool call** — the corrected source replaces `arguments.source` on the assistant message and `source` in the tool result, and the markdown mirror is refreshed, so a reload shows the fixed diagram with no trace of the failure. Module-level caches keep a known fix applied across remounts and guarantee **one** repair attempt per source — never a loop
- [x] **A failed repair is shown, not hidden** — if the model returns the same diagram, an empty one, or nothing usable, the command errors and the block falls back to the error view, which now says an automatic repair was tried and failed and still offers the manual "Ask model to fix" button that posts to the conversation
- [x] **Verified** — `cargo test` green (5 new tests for fenced-source extraction and tool-result patching); `npm run build` green; the print document generated against a fixture run covering plans, diagrams, plots, presented files, search hits, a failed tool, an unmapped MCP tool and a perspective zone
