...
- [x] A charting renderer taking a data spec (series, labels, axes, chart type) rather than a diagram source: bar, line, area, scatter, pie, stacked variants — [chart.ts](../src/lib/chart.ts), no charting dependency. `renderChartSvg` is a pure function from data to an SVG string, which is what lets the chat, the PDF export and the Markdown export all be served by one renderer. Grouped, stacked and horizontal bars; gaps (`null` is a gap, not a zero); a flat series; forty categories; a single slice filling the circle
- [x] Theme-aware and accessible by construction — the palette is read from the custom properties actually on `<html>`, so a repainted background is the background the axes are drawn against, and a `MutationObserver` over `class` and `style` keeps a chart correct when the theme changes with the answer still on screen. Series are separated on three channels, not one: hue, plus a dash pattern and marker shape for lines and points, plus a fill texture cut out of bars and slices in the surface colour. Series 0 takes the theme accent and stays plain; the other seven hues are fixed, so a chart does not change meaning when the user changes their accent
- [x] The model calls it with data it already has — `render_chart`, in the existing `render_graph` group so no zone config migrates. Like its siblings it validates and echoes; the two shapes it refuses are the ones that draw an empty frame (no series, and no numbers in any series), because a model reads a blank chart as a broken tool rather than as bad arguments
- [x] Charts survive export: PDF and Markdown export render them rather than dropping to a placeholder — the PDF draws the chart at the tool card's width from the export's own palette, which works precisely because the renderer needs no live document. The Markdown export writes the chart back as a ```chart fence: the same JSON the tool takes, which this app re-renders on import and which reads as the numbers themselves anywhere else. It is the one tool visual Markdown carries, and it earns that because a chart's content *is* data
- [x] A table in a model's markdown answer offers "chart this" — [DataTable.tsx](../src/components/Renderers/DataTable.tsx). The first non-numeric column names the rows and every numeric column becomes a series; where every column is numeric the first one is the labels, because a "Year | Revenue" table charting the year as a quantity is never what was meant

### 0.16.1 — Richer in-chat artifacts

- [x] Interactive tables: sort, filter, and copy from a rendered markdown table without leaving the chat — rendered from the markdown's own React children rather than from extracted text, so a cell keeps its links, code spans and emphasis and renders identically sorted or not; sorting is a permutation of those children. Reading a cell as a number is most of the work, because models write money, percentages, thousands separators and accounting negatives into tables constantly, and a "Cost" column sorted as text puts $99 above $1,240. Copy takes the rows on screen, not the rows as written
- [x] Inline results are addressable — a chart stages the ```chart fence that draws it into the composer, a table stages itself as Markdown, both appended to whatever is already typed. The reference *is* the data, so the next turn works from the numbers rather than re-deriving them. For a table this matters more than it looks: what is worth asking about is usually the filtered, sorted view, which exists nowhere in the transcript
- [x] **Somebody with no key and no local model can start.** Not in the original scope, and it should have been: the app's answer to "how do I begin" was *install Ollama*, which wants a capable machine, a download in gigabytes, and a tolerance for the result being slower and weaker than the free hosted tiers. Eight providers now carry a `freeTier` line in [providerPresets.ts](../src/lib/providerPresets.ts) — Google, Groq, Cerebras, OpenRouter, Mistral, Z.ai, NVIDIA and Cohere — each free without a payment method and about a minute from a key. First-run setup leads with them, ordered deliberately rather than alphabetically, each row linking straight to its own key page; Settings marks them; the skipped-setup banner names three. **There is deliberately no keyless hosted option**: an endpoint that answers without a key is either someone else's key being spent or a proxy reading every prompt, and neither belongs here — the only genuinely keyless providers are the local ones, which is the trade either way. And because recommending a hosted provider *is* the choice PRIVACY.md says the user gets to make, it is offered with what it costs, in `FREE_TIER_CAVEAT`: free tiers are generally the ones that train on what you send. The allowances are indicative and dated (checked August 2026) — the link beside each is the source of truth, so a stale number costs a rate-limit message rather than money. Two things checking rather than remembering caught: GitHub Models was retired on 30 July 2026, and Cloudflare Workers AI needs an account id inside its base URL, so neither is a one-click start
- [x] Markdown tables fill the message column rather than shrinking to their contents — a table is a comparison, and shrink-to-fit puts the columns being compared further apart than the eye wants them
- [x] **Audit what the existing renderers cost during streaming — done, and it found one thing worth fixing.** Block-level parsing (0.9.16) stopped a long answer re-parsing itself on every tick, but it changed nothing *inside* the trailing block, and that is where the cost was. An unclosed fence is already a code block to CommonMark, so a diagram being streamed is handed to its renderer at every intermediate length. Measured by replaying a 321-character answer through `splitMarkdownBlocks` four characters at a time: 80 tail re-parses, and **29 `mermaid.render` calls for one six-line diagram, 28 of them on source that cannot parse yet** — each a full parse, a failure, an error view and a DOM cleanup. The fix is [useSettledValue](../src/lib/useSettledValue.ts): the renderer sees the current value on mount and every later value only after 220 ms of quiet, so a streaming diagram is rendered once at the end and a finished one still appears instantly. Applied to Mermaid and to the legacy ```mathplot fence. The other two are already fine and it is worth recording why: `HtmlReportBlock` is keyed on a `present_file` path, which arrives complete, and a partially-streamed ```chart fence is invalid JSON, so it costs a failed `JSON.parse` and renders as code until it closes
...
- [x] Mock OpenAI-compatible streaming provider emitting a known token sequence — [mock-provider.mjs](../scripts/mock-provider.mjs) streams `0001 0002 …` one SSE frame per token with the knobs in the prompt (`tokens=`, `delay=`, `pause=`, `fail=`, `tool=`, `mix=`), and [token-sequence.mjs](../scripts/token-sequence.mjs) diffs what was stored against what was sent, naming the first missing index and telling a drop apart from a reorder. 16 offline tests, in `npm test`; the ones that matter are the negative controls, because a checker that always passes would close this item and prove nothing. [stream-soak.mjs](../scripts/stream-soak.mjs) runs the whole scenario set against a live app over the HTTP API — quick, sustained (100k), a mid-stream stall, seven concurrent, and a truncated stream as the control — and exits non-zero on a gap
...
- [~] Performance: measure and optimize startup time, first message render, large chat (500+ messages) scroll — **the instrument now exists and has to be pointed at something**: [perf.ts](../src/lib/perf.ts) adds launch marks and a scroll sampler (median/p95/worst frame time), inert unless `localStorage.mzPerf = "1"`, and [seed-chat.mjs](../scripts/seed-chat.mjs) builds a 500- or 2000-message chat with a stated, seeded content mix so two runs measure the same thing. Deliberately not Playwright — a browser download and a second runner to approximate a webview already on the machine. Procedure and thresholds in [RELEASE_TESTS.md](RELEASE_TESTS.md#2--performance--the-numbers-that-have-never-been-taken). **The decision it informs:** the message list is still not virtualised, and whether that matters at 500 or only at 2000 is now answerable rather than arguable — do not virtualise on suspicion, it interacts with scroll anchoring, streaming autoscroll, find-in-page and export. Previously: fixed the main structural cause of wasted re-renders: `Sidebar`/`ChatPanel`/`MessageThread`/`ChatList`/`ZoneEditor`/`ProjectsPanel`/`SettingsModal` subscribed to the whole zustand store unfiltered, so *any* state change anywhere re-rendered all of them; converted to shallow/per-field selectors, and `UserMessage`/`BotTurnView` are now memoized (with a custom comparator for `BotTurnView` since `groupMessages` rebuilds turn objects each call) so a streaming token only re-renders the turn actually generating, not the whole history. Still open: no virtualization for very long (500+) message lists, and no measured before/after startup or first-paint numbers.
- [x] Streaming: verify no dropped tokens or UI lag under sustained use — **run, on Windows 11 against 0.15.4, 2026-08-20: twelve scenarios, no dropped tokens.** 100k tokens in a single stream arrived complete and in order at ~33k tok/s; seven concurrent 20k streams each arrived complete (~37k tok/s aggregate, so concurrency is not contending for a shared buffer); a 1.5s mid-stream stall was not mistaken for the end of a turn; and the truncated control correctly refused to report a cut stream as complete. Memory held 128→137 MB with handles and threads flat across the run. What is *not* yet done is the 30-minute soak and the by-hand UI matrix in [RELEASE_TESTS.md](RELEASE_TESTS.md#1--no-dropped-tokens-under-sustained-use) — chat-switch mid-stream and stop mid-stream are the two the API path cannot see. Previously: **the check now exists and has to be run**: `node scripts/stream-soak.mjs` against the mock above, plus the by-hand UI matrix in [RELEASE_TESTS.md](RELEASE_TESTS.md#1--no-dropped-tokens-under-sustained-use) for the half an API driver cannot see. Previously: hardened `ChatPanel`'s Tauri event-listener effect to run once (refs instead of a dependency array) so the "stream" listener can never be torn down and re-attached mid-session, closing the only realistic drop window found. Sustained-use soak testing not yet done.
- [x] Installer: **Windows only for 1.0, and said so rather than left open** — NSIS ships today and is what every release has been built from. macOS and Linux are a distribution project, not a testing gap: an unsigned, un-notarised DMG gets a Gatekeeper block most people read as "broken", and notarisation needs an Apple Developer account at $99/year plus a signing identity in CI, which is a decision about who owns an account rather than an engineering task. Option 1 of the three in [TEST_STRATEGY.md](TEST_STRATEGY.md#3--installers-for-macos-and-linux) — ship Windows, size the signing work properly, and move both to 1.1
- [x] Auto-updater: Tauri updater plugin wired to GitHub releases — **the receiving half is confirmed, 2026-08-20**: an installed 0.15.5 found v0.15.6, downloaded it and restarted into it, which is the half three releases had never had anyone watch. Still worth doing once, separately: corrupt a character of a signature and confirm the manifest is *refused* — an updater that accepts a bad manifest is worse than one that never runs, and that path remains unexercised. Previously *built in 0.9.12*: plugin registered, signed `latest.json` published by CI alongside the installers, Settings → Data carries check / download-with-progress / install / restart. **The signing secrets are set** and the published manifest proves the whole CI half works: v0.13.0's entry is signed and its URLs are rewritten to the token-authenticated asset endpoints. What is still unverified is the *receiving* half — nobody has watched an installed build find an update, download it and restart into the new version. Several releases have now shipped (v0.12.6, v0.12.7, v0.13.0), so this needs no special setup: install the previous release, publish the next, and watch it. Local rehearsal without publishing anything is in [TEST_STRATEGY.md](TEST_STRATEGY.md#4--auto-updater-end-to-end)
- [~] Cross-platform smoke tests — **Windows 11 only, following the installer decision above.** There is nothing to smoke-test on the other two until there is something to install; the manual pass is [TEST_CHECKLIST.md](TEST_CHECKLIST.md) and its 🖥️ items need a genuinely fresh machine
...
- [x] **A dropped connection loses the partial answer; pressing stop keeps it.** — **fixed and verified**: `StreamAggregate` gained `error` as a sibling of `cancelled` and the failure arm breaks rather than returning, so the flush still runs and the partial reaches the caller; the call site persists it before failing the turn. The soak's truncated scenario stored 0 tokens before and stores 354 of the ~399 delivered now, and that assertion is part of the gate rather than a one-off. Original finding: Found by the truncated soak scenario: the mock delivered 399 of 5000 tokens and hung up, and the app stored *no assistant message at all* — not a partial, not an empty row. The engine is explicit that partial output should survive ([messages.rs](../src-tauri/src/commands/messages.rs), "Persist whatever the model produced before cancellation", with `skip_persist` firing only when content, reasoning and tool calls are all empty), so this is not a missing intention — a transport error propagates out before reaching that block. The result is an asymmetry the user cannot see the logic of: press stop and your half-answer is kept; have the provider drop and it is gone, after you watched it arrive. Against a local model on a flaky socket, or a hosted provider closing an idle connection, the dropped case is the *more* common one. The fix is to treat a stream error like a cancellation for persistence purposes and mark the turn failed, which is also what makes 0.9.x's "no tool reports a failure as an ordinary empty result" true of turns as well as tools. **Verified over the API path only** — the GUI streams through the same engine but nobody has watched a drop in the window, so it is worth confirming the frontend is not holding something the database is not
- [x] **A downgrade says what happened, in a sentence someone can act on.** — **fixed**: a guard runs before the migrator and reports cause, remedy and the fact that no data was touched, in place of sqlx's `VersionMissing` wording. Two tests, including one that it does not fire on an ordinary or fresh database. Original finding: Found by running a 0.12.7 binary against a database 0.15.4 had migrated: the app shows "MultiZone could not start", exits 1, and the reason is `migration error: migration 37 was previously applied but is missing in the resolved migrations`. Refusing to start is *right* — an old binary writing a newer schema is how a database gets corrupted, and there are 43 migrations to get wrong — but the sentence is written for whoever wrote sqlx, not for whoever is holding the machine. This matters at 1.0 specifically because 1.0 is the release that turns the updater on: a rollback, a second machine restored from a backup, or someone opening an older build they still had are all now reachable states, and each of them currently produces an error message whose only actionable content is a log path. Detect the case (applied migrations present that this build does not know) and say so — "This copy of MultiZone is older than the data on this machine. Install 0.15.4 or newer to open it." No data is at risk either way; this is entirely about the sentence
- [~] Clean uninstall: no orphaned files or registry entries — **the audit now exists and has to be run on a VM**: [uninstall-audit.ps1](../scripts/uninstall-audit.ps1) snapshots files and registry before install, after install and after uninstall, and prints what survived with our name on it. Every survivor is then a decision rather than a discovery — the database and the checkpoint blob store arguably should survive and NSIS should be asking; registry keys, the WebView2 user-data folder and anything under Program Files must not. Procedure in [RELEASE_TESTS.md](RELEASE_TESTS.md#4--clean-uninstall)
